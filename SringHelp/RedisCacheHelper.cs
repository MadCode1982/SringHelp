...
namespace SringHelp
{
    ...
    public class RedisCacheHelper
    {
        ...
        {
            redisSentinel = new RedisSentinel(ConfigurationManager.GetSection("RedisSentinelServers").Split(','));
            redisSentinel.Start();
            redisClientsManager = redisSentinel.GetRedisManager();
        }

        /// <summary>
        /// 手动更新 配置的 哨兵服务器
        /// </summary>
        public static void UpdateSentinel()
        {
            CreateManager();
        }
        ...
        {
            //using (redisClientsManager = redisSentinel.GetRedisManager()) { }
            using (IRedisClient redis = redisClientsManager.GetClient())
            {
